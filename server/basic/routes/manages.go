...
func Park_status_change(c *fiber.Ctx) error {
	...
	result := database.DB.Model(&park_entity).Select("park_id", "status", "gmt_modified").Updates(park_entity)
...
func Park_name_change(c *fiber.Ctx) error {
	...
	park_entity.Gmt_modified = time.Now()
	result := database.DB.Model(&park_entity).Select("park_id", "Park_name", "Gmt_modified").Updates(park_entity)
	if result.RowsAffected > 0 {
		return c.Status(200).SendString("更新成功")
	}
	return c.Status(400).SendString("更新失败")
